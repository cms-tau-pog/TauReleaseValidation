...
def matchObjectCollection3(objects, matchCollection, deltaRMax=0.3, filter=lambda x, y: True):
    ...
    allPairs = [(deltaR2(object.eta(), object.phi(), match.eta(), match.phi()), (object, match))
                for object in objects for match in matchCollection if filter(object, match)]
    allPairs.sort()
    #
...
def matchObjectCollection2(objects, matchCollection, deltaRMax=0.3):
    ...
    if len(matchCollection) == 0:
        return dict(zip(objects, [None]*len(objects)))
    # build all possible combinations
    allPairs = [(deltaR2(object.eta(), object.phi(), match.eta(), match.phi(
    )), (object, match)) for object in objects for match in matchCollection]
    allPairs.sort()
